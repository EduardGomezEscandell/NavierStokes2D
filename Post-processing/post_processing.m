function post_processing(coords, X_history, Pe_history, duration, dof, mesh,  corner_to_node)
    ...
    for step=1:mesh.steps+1
        ...
        subplot(1,3,2);
        t = trisurf(T1, coords(1,corner_to_node)', coords(2,corner_to_node)',  X(dof.p), X(dof.p));
        t.EdgeColor = 'None';
        shading interp
        c=colorbar('southoutside');
        ylabel(c,'Pressure');
        ...
        subplot(1,3,3);
        t = trisurf(T1, coords(1,corner_to_node)', coords(2,corner_to_node)', X(dof.d), X(dof.d));
        t.EdgeColor = 'None';
        shading interp
        c=colorbar('southoutside');
